...
class MainWindow3D(tk.Toplevel):
    def __init__(self, parent):
        ...
        self.geometry(f"{screen_width}x{screen_height}+0+0")  # Adjusts the window to top-left corner and matches screen size
    ...
    def create_buttons(self, style):
        ...
        buttons = [
            ("clean and body shape", self.finalto),
            ("Geometry cropping", self.finalto),
            ("3d face extraction", self.finalto),
            ("3d face mesh landmarks", self.finalto),
            ("left face region", self.finalto),
            ("right face region", self.finalto),
            ("3d landmarks", self.finalto),
            ("2d landmarks", self.finalto),
